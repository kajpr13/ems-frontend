...
const Notes = () => {
  ...
  const updateNote = async () => {
    try {
      if (currentNote.trim() !== '') {
        ...
        const response = await fetch(`https://ems-backend-production-9474.up.railway.app/editNote/3/43/${currentNote}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const deleteNote = async (index) => {
    try {
      if (currentNote.trim() !== '') {
        const response = await fetch(`https://ems-backend-production-9474.up.railway.app/deleteNote/3/${currentNote}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
